...
export async function fetchMarket(connection: Connection, address: PublicKey): Promise<Market> {
  const info = await connection.getAccountInfo(address);
  if (!info) throw new Error(`Market account not found: ${address.toBase58()}`);
  return deserializeMarket(info.data as Buffer);
}

export async function fetchOrder(connection: Connection, address: PublicKey): Promise<Order> {
  const info = await connection.getAccountInfo(address);
